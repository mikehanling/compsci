...
void simplefs_touch(dir_t * dir, char *fname){
  ...
  if(dir->nfiles == 0){ 
    ...
    file_t* new = calloc(1, sizeof(file_t));
    new->fname = name;
    new->next_file = NULL;
    time(&(new->last));
    dir->flist = new;
  ...
  while(cur_f != NULL){
    ...
    if (strcmp(cur_f->fname, fname) == 0) {
      *prev_f = cur_f->next_file;
      ...
      dir->nfiles--;
      return 1; //file found and removed
    }
...
void simplefs_ls_sorttime(dir_t * dir){
  ...
  for (int i = 0; i < dir->nfiles; i++) {
    while (curr) {
      if (difftime(curr->last, old->last) < 0){// && 
          //difftime(older->last, curr->last) >= 0)
        printf("IN IF\n");
        old = curr;
      }
      curr = curr->next_file;
    }
    simplefs_touch(sorted, old->fname);
    older = old;
    curr = dir->flist;
  }
  printf("%d\n", sorted->nfiles);
