...
function test_part2(){
    ...
    cd part2
    rm -f creditcardcheck
    cmd="gcc -Wall -g creditcardcheck.c -o creditcardcheck 2>&1"
    res=$(eval $cmd)
    utest_z "$res" "Compiles without warnings or errors ($cmd)"
    if [ ! -z "$res" ]
    then
    ...
    cmd='./creditcardcheck $card1'
    res=$(eval $cmd)
    expect="VALID"
    utest "$res" "$expect" "Random credit card 1 passes ($cmd)"

    cmd='./creditcardcheck $card2'
    res=$(eval $cmd)
    expect="VALID"
