...
void read(int* array, int size, FILE* in) {
  char currchar;
  fscanf(in, "%c", &currchar);
  int numspace = 0;

  //tracks where in the char row you are
  //increases value of the column if there is a #
  do {  
    numspace++;  
    if (currchar == '#') {    
      array[(numspace/2)-1] += 1;
    }  //end if
    ...
    fscanf(in, "%c", &currchar);
  } while (currchar != '-');  //end dowhile

  //reads in the useless dashes and column labels
  for (int i = 0; i < (2*size); i++) {
    fscanf(in, "-");
  }  //end for
  char trash;
  for (int j = 0; j < size; j++) {  //space and char
    fscanf(in, " %c", &trash);
  }  //end for
